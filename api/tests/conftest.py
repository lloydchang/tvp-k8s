...
from app.index import app  # Import the FastAPI app instance from index.py instead of main.py
...
def mock_kubernetes_client():
    ...
    with patch("app.index.get_kubernetes_client") as mock_client: # Target app.index
...
def mock_argo_cd_token():
    ...
    with patch("app.index.get_argo_cd_token") as mock_token: # Target app.index
...
def test_health_check_kubernetes_unhealthy():
    ...
    with patch("app.index.get_kubernetes_client") as mock_client:
        kubernetes_client = MagicMock()
...
def test_health_check_argo_cd_unhealthy():
    ...
    with patch("app.index.get_argo_cd_token") as mock_token:
