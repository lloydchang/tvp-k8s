...
def test_sanitize_branch_name_with_import_error():
    """Test the branch name sanitization function when re module has issues"""
    from python.app.gitops import sanitize_branch_name
    
    # Mock re.sub to simulate an error
    with patch("re.sub") as mock_re_sub:
        # Simulate an attribute error or other issue
        mock_re_sub.side_effect = AttributeError("Module 're' has no attribute 'sub'")
        
        # The function should handle the error gracefully and return the original string
        # or at least not raise an exception
        result = sanitize_branch_name("test-branch")
        assert isinstance(result, str)  # Should still return a string

def test_sanitize_git_url_with_import_error():
    """Test the git URL sanitization function when re module has issues"""
    from python.app.gitops import sanitize_git_url
    ...
    with patch("re.sub") as mock_re_sub:
        ...
        # The function should handle the error gracefully and return the original string
        # or at least not raise an exception
        result = sanitize_git_url("git@github.com:user/repo.git")
        assert isinstance(result, str)  # Should still return a string

def test_clone_repository_with_validation_error():
    """Test repository cloning with sanitization errors"""
    from python.app.gitops import _clone_repository
    
    # Mock sanitize functions to raise an exception
    with patch("python.app.gitops.sanitize_branch_name") as mock_sanitize_branch, \
         patch("python.app.gitops.sanitize_git_url") as mock_sanitize_url, \
         patch("os.makedirs"):
        
        # Configure first mock to raise an exception
        mock_sanitize_branch.side_effect = ValueError("Invalid branch name")
        mock_sanitize_url.return_value = "https://github.com/user/repo.git"
        
        # Call the function and expect the error to be propagated
        with pytest.raises(ValueError):
            _clone_repository("https://github.com/user/repo.git", "/tmp/repo", "invalid&branch")
            
        # Reset and test URL sanitization error
        mock_sanitize_branch.reset_mock()
        mock_sanitize_branch.side_effect = None
        mock_sanitize_branch.return_value = "main"
        mock_sanitize_url.side_effect = ValueError("Invalid URL")
        
        with pytest.raises(ValueError):
            _clone_repository("invalid://url", "/tmp/repo", "main")

def test_apply_configurations_manifests_errors():
    """Test apply configurations with manifests directory errors"""
    from python.app.gitops import _apply_configurations_from_git
    ...
    # Mock manifests directory
    manifests_dir = MagicMock()
    manifests_dir.exists.return_value = True
    manifests_dir.is_dir.return_value = True
    
    # Setup file paths with a special __truediv__ that raises exceptions for specific paths
    def mock_truediv(self, other):
        if other == "values.yaml":
            return values_file
        elif other == "manifests":
    ...
    # Test case where subprocess.run raises an exception
    with patch("builtins.open", mock_open(read_data="image: test-image")), \
         patch("yaml.safe_load") as mock_yaml_load, \
         patch("subprocess.run") as mock_run:
        ...
        # Simulate subprocess error during kubectl apply
        mock_run.side_effect = subprocess.CalledProcessError(
            returncode=1,
            cmd=["kubectl", "apply"],
            output=None,
            stderr="Error: unable to apply manifests"
        )
        
        # Call the function - it should handle the error
        _apply_configurations_from_git(mock_repo_path)
        
        # This test just verifies that the function handles the error without raising it

def test_reconcile_from_git_complex_errors():
    """Test reconcile_from_git with more complex error scenarios"""
    from python.app.gitops import reconcile_from_git
    import threading
    
    # Create some complex test scenarios that might be hard to trigger otherwise
    
    # Test 1: Test thread lock handling with mock
    with patch("threading.Lock") as mock_lock_class:
        # Create a mock lock that raises an exception when used in a with statement
        mock_lock = MagicMock()
        mock_lock.__enter__.side_effect = threading.ThreadError("Lock acquisition failed")
        mock_lock_class.return_value = mock_lock
        
        # Import the module again to use our mocked lock
        with patch("python.app.gitops.reconciliation_lock", mock_lock):
            # The function should handle lock errors
            reconcile_from_git()  # Should not raise an exception
            
    # Test 2: Various Git operation errors
    with patch("python.app.gitops._clone_repository") as mock_clone, \
         patch("pathlib.Path.exists") as mock_exists, \
         patch("python.app.gitops.reconciliation_lock", MagicMock()):
        
        # Simulate repo doesn't exist, but clone fails with generic Exception
        mock_exists.return_value = False
        mock_clone.side_effect = Exception("Unknown error during Git clone")
